...
let s = '\ufeff' + 'participant,bias,set,trial,targetW,targetX,targetY,touchX,touchY,distractorW,distractorP,MT,Error,d,attempt'
let attempt = 0
  ...
  for (const data of dataArray) {
    with (data) {
      ...
      if (trial !== -1 /* && ptrial != trial*/) {
        ...
        attempt = ptrial == trial ? attempt + 1 : 0
        s += '\n' + pnum + ',' + bias + ',' + set + ',' + trial + ',' + targetW + ',' + tx + ',' + ty + ',' + mx + ',' + my + ',' + distractorW + ',' + distractorP + ',' + MT + ',' + err + ',' + d + ',' + attempt
      }
      if (!err) {
        ptime = time
      }
